...
public:
    enum Method
    {
        kInvalid, kGet, kPost, kHead, kPut, kPatch, kDelete, kOptions
    };
    ...
    {
        auto it = contextValues_.find(key);
    ...
    {
        return contextValues_.find(key) != contextValues_.end();
    }
...
private:
    ...
    std::unordered_map<std::string, std::string> contextValues_;
};  
